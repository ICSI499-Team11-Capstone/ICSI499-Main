...
from utils.kfoldtrainer import Trainer
...
class SequenceTrainer(Trainer):
    ...
    def loss_and_acc_for_batch(self, batch, c4_labels,edge_index, epoch_num=None, batch_num=None, train=True, weightedLoss=False, purity_flag=False, probBins=[]):
        """
        ...
        if torch.cuda.is_available():
            inputs = inputs.cuda()
        # outputs, latent_dist, prior, latent_sample, _, mean_z, std_z = self.model(inputs, edge_index)
        outputs, latent_sample, z_dist, prior, z_mean, z_log_std = self.model(inputs, edge_index)
 
       # print("Z-Mean is--------------Z-Mean-----: ",len(z_mean))
#        print("Z-edge is--------------Z-edge-----: ",np.array(edge_index)[:,:2])
#        print("Z-translate is--------------Z-translate-----: ",translation)
        # compute accuracy
        ...
        purity = 0
        if purity_flag:
            # print("labels   ", labels.cpu().numpy().reshape(-1))
            # print("c4 labels   ", c4_labels)
            # purity = SequenceTrainer.calculate_purity(latent_sample.detach().cpu().numpy(), labels.cpu().reshape(-1), c4_labels)
            purity = SequenceTrainer.calculate_purity(latent_sample.detach().cpu().numpy(), c4_labels)
    ...
    @staticmethod
    def calculate_purity(latent_code, class4_labels, k=10):
        """
