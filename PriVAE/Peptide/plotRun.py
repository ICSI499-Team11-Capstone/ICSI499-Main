...
def genFigure(filePath):
    ...
    # fig.suptitle(r" $\beta$=" + str(beta) + r" $\gamma$=" + str(gamma) + " latentDims=" + str(latentDims) + " lstmLayers=" + str(lstmLayers) +
    #              " Dropout=" + str(drop) + " lstmInfo=" + str(lstmInfo) + " linearWidth", fontsize=12)
    fig.suptitle(
    r"$\beta$=" + f"{beta:.3f}" +
    r" $\gamma$=" + f"{gamma:.3f}" +
    ...
    ax[2].set_ylim([0, 1])  # Set the y-axis range for accuracy
    ...
    # Check if accuracy arrays are valid and add accuracy text in the middle of the accuracy plot (ax[2])
